/**
 * VEYa — Screen 09: Purpose Selection ✨
 *
 * Users choose what brings them to VEYa. Multi-select cards
 * with the same luxury visual language as methodology screen.
 *
 * @module screens/onboarding/purpose
 * @version 1.0.0
 */
...
const PURPOSES: PurposeItem[] = [
  {
    ...
    emoji: '💕',
    label: 'Love &\nRelationships',
    description: 'Navigate romance with cosmic clarity',
    accentColor: '#E8788A',
  },
  {
    id: 'career',
  ...
  },
  {
    ...
    emoji: '🌱',
    label: 'Personal\nGrowth',
    description: 'Deepen self-understanding',
    accentColor: '#6B8E6B',
  },
  {
    id: 'wellness',
  ...
  },
  {
    ...
    emoji: '💰',
    label: 'Financial\nGuidance',
    description: 'Cosmic timing for money decisions',
    accentColor: '#D4A547',
  },
  {
    id: 'spiritual',
  ...
  },
  {
    ...
    emoji: '👥',
    label: 'Family &\nFriendships',
    description: 'Understand your closest bonds',
    accentColor: '#C4A0E8',
  },
  {
    id: 'daily',
    emoji: '⚡',
    label: 'Daily\nGuidance',
    description: 'Just help me navigate each day',
    accentColor: '#E8664D',
  },
];
...
const PurposeCard = ({
  ...
  item: PurposeItem; isSelected: boolean; onToggle: (id: string) => void; index: number;
}) => {
  ...
  return (
    <Animated.View entering={FadeIn.duration(300)} style={[styles.counterContainer, counterAnimatedStyle]}>
      <Text style={styles.counterText}>
        {count} {count === 1 ? 'goal' : 'goals'} selected
      </Text>
    </Animated.View>
  );
}
...
function PurposeScreenInner() {
  ...
  const gridRows = useMemo(() => {
    const rows: PurposeItem[][] = [];
    for (let i = 0; i < PURPOSES.length; i += 2) {
      rows.push(PURPOSES.slice(i, i + 2));
    }
    return rows;
  }, []);
      ...
      <View style={[styles.content, {
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}
          ...
          <Animated.Text entering={FadeInDown.duration(500).delay(500).easing(Easing.out(Easing.ease))}
            ...
            Understanding your goals helps{'\n'}VEYa guide you better
          </Animated.Text>
            ...
            {gridRows.map((row, rowIndex) => (
              <View key={rowIndex} style={styles.gridRow}>
                {row.map((item, colIndex) => (
                  <PurposeCard key={item.id} item={item}
        ...
        <Animated.View entering={FadeInUp.duration(500).delay(1400).easing(Easing.out(Easing.ease))}
...
const styles = StyleSheet.create({
  ...
  cardEmoji: { fontSize: 28, marginBottom: spacing.xs, lineHeight: 34 },
  cardLabel: {
    fontFamily: typography.fonts.bodyMedium, fontSize: typography.sizes.caption,
    color: colors.textPrimary, textAlign: 'center', lineHeight: typography.sizes.caption * 1.35,
    letterSpacing: 0.1,
  },
...
export default function PurposeScreen() {
  return (
    <ScreenErrorBoundary screenName="Purpose">
      <PurposeScreenInner />
    </ScreenErrorBoundary>
  );
}
